...
const PhotoGrid = () => {
  ...
  const handleView = (download_link, description) => {
    ...
    setPhotoContent(null); // 👈 clear previous photo
  ...
  const handleDelete = (photo_id) => {
    ...
    if (isConfirmed) {
      ...
      fetchDeleteDataWithAuth('/albums/' + album_id + '/photo/' + photo_id + '/delete').then((res) => {
        console.log(res);
        ...
        <div className={classes.modalMain}>
          {photoLoading ? (
            <div style={{ textAlign: 'center', padding: '40px' }}>
              <Typography variant="h6">Loading photo…</Typography>
          ...
          <Stack direction="row" spacing={2} justifyContent="flex-end" sx={{ mt: 2 }}>
            <Button variant="contained" color="success" startIcon={<DownloadOutlined />} onClick={() => handleDownload(DownloadLink)}>
              Download
            </Button>
